import { ArrowLeft, Package, Download, Star, Users, Search, Filter, Shield, Zap, Code, BookOpen, ExternalLink, CheckCircle, AlertTriangle } from "lucide-react";
import { Link } from "react-router-dom";

const UURDocs = () => {
  const featuredPackages = [
    { name: "urbanshade-themes", description: "Custom theme packs for UrbanShade OS", downloads: "2.4k", stars: 48, author: "aswdBatch" },
    { name: "facility-sounds", description: "Ambient sound effects and alerts", downloads: "1.8k", stars: 32, author: "community" },
    { name: "extended-terminal", description: "Additional terminal commands and utilities", downloads: "3.1k", stars: 67, author: "defdev-team" },
    { name: "custom-bugchecks", description: "Create your own bugcheck screens", downloads: "890", stars: 21, author: "community" },
  ];
      ...
      <header className="sticky top-0 z-50 border-b border-cyan-500/20 bg-slate-950/90 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs px-2 py-0.5 bg-cyan-500/20 text-cyan-300 rounded-full">v1.0</span>
          </div>
          <Link 
            to="/docs" 
      ...
      <main className="max-w-6xl mx-auto px-6 py-12 space-y-16">
        ...
        <section className="space-y-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-xl">
              ...
              <p className="text-sm text-slate-400 leading-relaxed">
                Packages can be installed directly through the App Store application in UrbanShade OS, 
                or via the terminal using the <code className="px-1.5 py-0.5 bg-slate-800 rounded text-cyan-400">uur</code> command-line tool.
              </p>
            </div>
          </div>
        </section>
              ...
              <div key={pkg.name} className="p-5 bg-slate-900/50 border border-slate-800 rounded-xl hover:border-cyan-500/30 transition-colors">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">by {pkg.author}</p>
                  </div>
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="p-6 bg-slate-900/50 border border-slate-800 rounded-xl">
              <h4 className="font-bold text-slate-100 mb-4">Via Terminal</h4>
              <div className="space-y-3 font-mono text-sm bg-slate-950 p-4 rounded-lg">
                <p><span className="text-green-400">$</span> uur search <span className="text-cyan-400">[package-name]</span></p>
                <p><span className="text-green-400">$</span> uur install <span className="text-cyan-400">[package-name]</span></p>
                <p><span className="text-green-400">$</span> uur update <span className="text-cyan-400">[package-name]</span></p>
                <p><span className="text-green-400">$</span> uur remove <span className="text-cyan-400">[package-name]</span></p>
                <p><span className="text-green-400">$</span> uur list <span className="text-slate-500"># List installed packages</span></p>
              </div>
            </div>
